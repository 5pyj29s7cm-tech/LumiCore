...
export function MemoryAvatarPortraitStage({ ownerId, avatarId, mediaId, stream, speaking, name, locale }: {
  ownerId: string; avatarId: string; mediaId: string; stream: MediaStream | null; speaking: boolean; name: string; locale: 'zh' | 'en';
}) {
  ...
  return <div className="relative h-full w-full bg-[#17201f]">
    ...
    <video ref={video} autoPlay playsInline aria-label={copy.label} className={`absolute inset-0 h-full w-full object-contain ${stream && speaking ? '' : 'invisible'}`} />
