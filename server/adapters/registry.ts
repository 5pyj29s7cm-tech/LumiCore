...
export function getAdapterRegistry(options: AdapterRegistryOptions = {}): AdapterRegistryReport {
  ...
  const adapters: AdapterCapability[] = [
    ...
    },
    {
      ...
      category: 'client',
      status: !hasState ? 'requires_setup' : staleState ? 'attention' : 'ready',
      actions: ['design_delivery_workflow', 'design_delivery_panel', 'close_design_delivery_panel', 'client_action', 'desktop_show_lumi_window', 'desktop_set_wallpaper_mode', 'desktop_cursor_glow_show', 'desktop_cursor_glow_update', 'desktop_cursor_glow_click', 'desktop_cursor_glow_hide', 'desktop_active_window', 'desktop_capture_screen', 'desktop_list_files', 'desktop_open', 'desktop_run_command', 'desktop_clipboard_write', 'desktop_keyboard_press', 'create_ppt', 'create_pdf', 'cad_generate_dxf', 'cad_prepare_autocad_operations', 'mcp_cad-drafting_autocad_playback_file'],
      surfaces: ['Lumi desktop', 'large design delivery panel', 'WPS or editor', 'client-facing PPT/PDF proposal files', 'desktop CAD apps', 'CAD DXF draft', 'AutoCAD MCP/COM stroke-by-stroke playback', 'Revit/Dynamo handoff files', 'personal WeChat or WeCom'],
      requiresConfirmation: false,
      ...
      notes: 'Use for current-stage industry videos and real design delivery tasks where Lumi turns a design request into external-system artifacts: proposal, budget/material list, client-facing design proposal PPTX/PDF with layout/material/budget visuals, explicit CAD file exports, AutoCAD MCP/COM playback that creates entities stroke by stroke with completion-marker verification, Revit/Dynamo handoff files, and a WeChat delivery draft. Visible AutoCAD work never falls back to generated files, LISP, scripts, or batch commands. Prefer restoring an already logged-in personal WeChat window before falling back to WeCom. Treat the video flow as a reusable delivery standard, not a fixed script: derive the deliverables from the current client/project and use the available external tools to reach a visible result.',
    },
    {
    ...
    },
    {
      ...
      notes: 'Persistent task hub for customer, store, account, case-filing, video-publishing, and design-delivery takeover work. Lumi core should stay thin: receive/parameterize the task, orchestrate safe steps, choose the industry package adapter, verify outputs, and stop at confirmation boundaries. Industry content is prepared through work_takeover_task_prepare_industry_package, which routes to ecommerce/short-video/account, renovation/CAD/Revit, and future skill-backed packages without turning the core task loop into one fixed industry script. It can safely advance to the next unprepared step from execution history, run a bounded autorun loop from message/clipboard/task to local packet, run work_takeover_capability_reuse_probe to pressure-test whether the task is reusing learned routes/adapters/tools/skills instead of growing duplicate capabilities, or run work_takeover_real_smoke_run for a true closed-loop test that selects external-control routes, prepares supported industry packages, verifies desktop/file/content/draft evidence, exports a local packet, and writes a concise human report back to the task. It can verify the result against current desktop/window/process/screenshot state, local files, drafts, and artifact content terms, write artifacts, drafts, verification notes, and next instructions back to the task, export a local task packet so the result exists as files, and run one explicitly selected plan-suggested tool through the normal confirmation gate.',
    },
    {
    ...
    },
    {
      ...
      actions: ['wechat_intake_analyze', 'wechat_intake_from_clipboard', 'work_takeover_task_from_wechat', 'work_takeover_task_from_clipboard', 'wechat_prepare_reply', 'wechat_copy_reply_draft', 'wechat_read_recent_chat', 'wechat_send_message', 'wechat_send_file', 'messaging_list_file_targets', 'feishu_send_file'],
      surfaces: ['WeChat', 'Feishu bot/remote channel', 'cross-workspace file transfer', 'court-notice link intake', 'clipboard drafts', 'work takeover intake'],
      requiresConfirmation: false,
      diagnostics: [`sendRequiresConfirmation=${gate.messagingSendRequiresConfirmation}`],
      safety: 'Lumi can read user-provided or copied message text, read visible foreground WeChat chats with screenshot/OCR/UI evidence, triage messages into work takeover, persist a task, draft replies, copy drafts, and send ordinary foreground user-requested replies/messages when supervised. Payments, login/security verification, account switching, legal/contractual commitments, and other high-consequence sends stay confirmation-gated.',
      notes: 'Use WeChat intake as the front door for customer, store, account, case-filing, video-publishing, and design-delivery takeover tasks. Treat reading, drafting, and sending as separate actions with separate evidence; do not claim chat content was read unless foreground visual evidence or the read-chat tool completed. Personal WeChat may enter an organization only through the bound member identity: uniquely matched court notices can be archived directly, ambiguous cases ask once, and cross-workspace file sends are target-bound and audit-logged. Feishu and WeCom group members are routed independently by member, chat, and thread.',
    },
    {
      id: 'cad_bim.drafting',
      ...
      category: 'cad_bim',
      status: 'ready',
      actions: ['floorplan_extract_geometry', 'ocr_image_file', 'cad_generate_dxf', 'cad_prepare_autocad_operations', 'mcp_cad-drafting_autocad_playback_file', 'design_delivery_workflow', 'mcp_cad-drafting_cad_renovation_folder_workflow'],
      surfaces: ['runtime logs', 'CAD handoff files', 'desktop CAD apps', 'AutoCAD MCP/COM stroke-by-stroke playback', 'large design delivery panel', 'client-facing proposal deck'],
      requiresConfirmation: false,
      safety: 'Generated DXF/IFC/BIM drafts and visible CAD playback can run under the active desktop mode, but they are not production drawings until dimensions and standards are reviewed.',
      notes: 'Current stable path separates file deliverables from visible app execution. Renovation design delivery can package proposal, budget, customer-ready PPTX/PDF reports, explicit DXF drafts, AutoCAD MCP/COM entity playback with completion-marker verification, and Revit/Dynamo handoff files. A generated file cannot substitute for failed visible AutoCAD playback. Direct native RVT production output still requires a confirmed Revit adapter.',
    },
    {
