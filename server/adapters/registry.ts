...
export function getAdapterRegistry(options: AdapterRegistryOptions = {}): AdapterRegistryReport {
  ...
  const adapters: AdapterCapability[] = [
    ...
    },
    {
      ...
      actions: ['wechat_intake_analyze', 'wechat_intake_from_clipboard', 'work_takeover_task_from_wechat', 'work_takeover_task_from_clipboard', 'wechat_prepare_reply', 'wechat_copy_reply_draft', 'wechat_read_recent_chat', 'wechat_send_message'],
    ...
    },
    {
      ...
      category: 'cad_bim',
      status: 'ready',
      actions: ['floorplan_extract_geometry', 'ocr_image_file', 'cad_generate_dxf', 'cad_generate_autocad_draw_script', 'cad_run_autocad_draw_script', 'design_delivery_workflow', 'mcp_cad-drafting_cad_renovation_folder_workflow'],
      surfaces: ['runtime logs', 'CAD handoff files', 'desktop CAD apps', 'AutoCAD stroke-by-stroke script', 'large design delivery panel', 'client-facing proposal deck'],
      requiresConfirmation: false,
      ...
      notes: 'Current stable path is file generation plus active external-tool handoff. Renovation design delivery can package proposal, budget, customer-ready PPTX/PDF reports with visual design content, DXF drafts opened in FreeCAD/AutoCAD-compatible tools when available, AutoCAD LISP/SCRIPT playback that visibly draws the plan stroke by stroke and can be launched/verified, and Revit/Dynamo handoff files. Direct native RVT production output still requires a confirmed Revit adapter.',
    },
    {
    ...
    },
    {
      ...
      actions: ['desktop_ai_list_targets', 'desktop_ai_ask', 'desktop_ai_collect_answer', 'external_app_list_adapters', 'adapter_registry_list', 'capability_research', 'computer_use'],
      surfaces: ['MCP', 'browser', 'files', 'clipboard', 'local AI apps', 'WorkBuddy', 'Codex desktop'],
      requiresConfirmation: false,
      ...
      notes: 'Lumi can research, draft adapters, ask desktop AI apps such as WorkBuddy/Codex through real windows, collect visible answers with screenshot/vision evidence, and coordinate connected AI apps without per-tool permission popups. API/MCP/CLI integrations are preferred when available; installing or running untrusted third-party code remains a hard boundary.',
    },
    {
      id: 'ai.nano_banana',
