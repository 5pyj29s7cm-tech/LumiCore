...
export function getAdapterRegistry(options: AdapterRegistryOptions = {}): AdapterRegistryReport {
  ...
  const adapters: AdapterCapability[] = [
    ...
    },
    {
      ...
      category: 'automation',
      status: 'available',
      actions: ['desktop_active_window', 'desktop_capture_screen', 'desktop_open', 'desktop_run_command', 'web_login_profile_list', 'web_login_profile_save', 'web_login_learn_site', 'web_login_run', 'browser_open_task'],
    ...
    },
    {
      ...
      safety: 'Lumi can read user-provided or copied message text, read visible foreground WeChat chats with screenshot/OCR/UI evidence, triage messages into work takeover, persist a task, draft replies, copy drafts, and send ordinary foreground user-requested replies/messages when supervised. Payments, account-security actions, legal/contractual commitments, and other high-consequence sends stay confirmation-gated.',
