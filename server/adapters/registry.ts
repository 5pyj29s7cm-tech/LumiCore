...
export function getAdapterRegistry(options: AdapterRegistryOptions = {}): AdapterRegistryReport {
  ...
  const adapters: AdapterCapability[] = [
    ...
    },
    {
      ...
      actions: [
        'open_organization_workspace(section=dashboard|kb|chat|messaging|members|audit|settings|branch|legal|spatial-design|brand-design)',
      ],
      surfaces: [
    ...
    },
    {
      ...
      requiresConfirmation: true,
      setup: skillStats.connected > 0 ? [] : ['Connect one specific AI app, MCP server, browser account, or file workflow before using it as a LumiCore tool.'],
      notes: 'LumiCore may use one explicitly selected external AI application as a bounded tool. It keeps ownership of the task and reads back evidence from that same target. External AI history uses a separate authorization ledger with exact source scopes, durable cursors, and source evidence. Installing or running untrusted third-party code remains a hard boundary.',
    },
    {
      id: 'ai.nano_banana',
    ...
    },
    {
      ...
      surfaces: ['explicitly paired Lumi instances'],
      requiresConfirmation: true,
      safety: 'Every LAP session is explicit, scoped, revocable, and separated from local LumiCore memory and identity.',
      notes: 'LAP is a user-authorized, scoped, and revocable protocol between separately owned Lumi instances.',
    },
    {
