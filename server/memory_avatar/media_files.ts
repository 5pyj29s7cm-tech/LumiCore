...
import { runMediaProcess as runSharedMediaProcess } from '../media/process';
...
/** Preserve avatar error responses while sharing process ownership and cancellation. */
export function runMediaProcess(binary: 'ffmpeg' | 'ffprobe', args: string[], signal?: AbortSignal, timeoutMs = 120_000): Promise<string> {
  return runSharedMediaProcess(binary, args, signal, timeoutMs, mediaError);
}
