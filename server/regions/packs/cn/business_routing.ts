...
export function businessToolHints(text: string): string[] {
  ...
  if (/订单|店铺|电商|销售额|ecommerce|store-data|today-operations/iu.test(text)) names.push('business_ecommerce_ecommerce_order_profit', 'business_ecommerce_platform_settlement_reconcile', 'industry_ecommerce_today_snapshot', 'industry_ecommerce_store_data_snapshot', 'business_finance_ecommerce_tax_workpaper');
  ...
  if (/评论|评价|review.*insight/iu.test(text)) names.push('business_ecommerce_review_source_normalizer', 'business_ecommerce_review_insight_analyzer');
