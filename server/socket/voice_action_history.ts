...
import { hasVisibleAutoCadExecutionEvidence } from '../cognition/action_contract';
...
export function describeRecentActionsFromHistory(text: string, history: any[]): string | null {
  ...
  if (wantsCad && cadCalls.length > 0) {
    const visiblePlayback = hasVisibleAutoCadExecutionEvidence(cadCalls as ToolExecutionRecord[], text);
    if (!visiblePlayback) {
