...
const CAPABILITY_RULES: CapabilityRule[] = [
  ...
  },
  {
    ...
    label: 'Customer sales and service operations',
    kind: 'automation',
    tools: ['mcp_sales-customer-ops_lead_score', 'mcp_sales-customer-ops_sales_followup_draft', 'mcp_sales-customer-ops_objection_response_builder', 'mcp_sales-customer-ops_customer_health_review', 'mcp_sales-customer-ops_support_ticket_triage', 'wechat_send_message', 'work_product_verify'],
    confirmationRequired: ['Confirm final commercial terms, binding commitments, signatures, and other high-consequence customer actions.'],
    keywords: ['customer', 'lead', 'sales', 'follow-up', 'objection', 'support', 'quote', 'client'],
    categoryHints: ['customer'],
  },
  {
    id: 'ecommerce.operations',
    label: 'Ecommerce data and platform operations',
    kind: 'automation',
    tools: ['mcp_ecommerce-ops_product_listing_optimizer', 'mcp_ecommerce-ops_ecommerce_order_profit', 'mcp_ecommerce-ops_inventory_restock_plan', 'mcp_ecommerce-ops_platform_settlement_reconcile', 'mcp_ecommerce-ops_campaign_roi_analyzer', 'mcp_ecommerce-ops_after_sales_risk_report', 'web_login_run', 'mcp_playwright_browser_snapshot', 'work_product_verify'],
    confirmationRequired: ['Confirm price, inventory, ad-spend, payment, purchase, and other high-consequence platform changes.'],
    keywords: ['ecommerce', 'store', 'listing', 'order', 'inventory', 'campaign', 'settlement', 'SKU'],
  ...
  },
  {
    ...
    label: '文档/方案/报价材料',
    kind: 'document',
    tools: ['create_docx', 'create_xlsx', 'create_pdf', 'work_product_verify'],
  ...
  },
  {
    ...
    label: 'CAD/BIM 交付物',
    kind: 'external_app',
    tools: ['desktop_list_files', 'read_file', 'read_pdf', 'ocr_image_file', 'floorplan_extract_geometry', 'cad_generate_dxf', 'cad_prepare_autocad_operations', 'mcp_cad-drafting_autocad_playback_file', 'adapter_execute', 'desktop_open', 'work_product_verify'],
...
const CATEGORY_CAPABILITY_HINTS: Partial<Record<WechatWorkCategory, string[]>> = {
  customer: ['customer.operations', 'document.proposal_packet', 'messaging.reply_handoff'],
  store: ['ecommerce.operations', 'browser.account_platform_work', 'messaging.reply_handoff', 'document.proposal_packet'],
  account: ['ecommerce.operations', 'browser.account_platform_work', 'video.content_publish_pack', 'document.proposal_packet'],
  legal_case: ['legal.case_filing_pack', 'messaging.reply_handoff'],
  video_publish: ['ecommerce.operations', 'video.content_publish_pack', 'browser.account_platform_work', 'presentation.client_deck'],
