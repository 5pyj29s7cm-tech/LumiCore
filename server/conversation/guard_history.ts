 ...
 * result, not a factual assistant answer. The customer display boundary turns
 * legacy rows into natural copy, while prompt and summary paths quarantine
 * them so the model cannot learn the verifier's implementation vocabulary.
 */
...
export function isGuardGeneratedAssistantText(value: unknown): boolean {
  return containsInternalExecutionLanguage(value)
    || GUARDED_ASSISTANT_TEXT_RE.test(String(value || ''));
}
