...
import { isOwnerInteractionEvidence } from './provenance';
...
export function analyzeBehavioralPatterns(userId: string, domain = 'personal', orgId = ''): BehavioralPattern[] {
  ...
  const interactions = (db.interactions || []).filter((i: any) => {
    ...
    return i.userId === userId && isOwnerInteractionEvidence(i);
  }).slice(-100);
  ...
  const ownerMessages = interactions.filter((i: any) => !i.role || i.role === 'user');
  const hourCounts = new Map<number, number>();
  for (const i of ownerMessages) {
    if (i.timestamp) {
  ...
  if (hourCounts.size > 0) {
    ...
    const totalInteractions = ownerMessages.length;
  ...
  for (const i of interactions) {
    if (Array.isArray(i.toolCalls)) {
      for (const tc of i.toolCalls) {
        if (!tc || typeof tc !== 'object' || !/^[a-z][a-z0-9_-]{1,127}$/i.test(tc.name || '') || !toolRecordSucceeded(tc)) continue;
        const name = tc.name;
  ...
  const avgPerDay = ownerMessages.length / Math.max(1, dates.size);
  if (dates.size >= 3) {
    patterns.push({
