...
export const OPERATION_MODE_CONFIGS: Record<OperationMode, OperationModeConfig> = {
  ...
  assistant: {
    ...
    promptOverlay: [
      ...
      'For supervised foreground work, ordinary user-requested messages, comments, replies, non-commercial content posts, and saved/authorized login session reuse can proceed; payments, purchases, transfers, first-time login, QR/OTP/captcha/passkey/security verification, account switching, credential storage, third-party authorization, legal filings/signatures, ambiguous submits, and destructive/system actions still require confirmation or handoff.',
      ...
      'Trusted desktop launches, foreground app focus, and CAD playback handoffs may run without a separate permission prompt when requested by the user; generic shell, installs, package changes, git mutations, and destructive commands still stop at the action boundary.',
    ].join('\n'),
    toolPolicy: {
      allowedTools: ['*'],
      requireConfirmation: [
  ...
  autonomous: {
    ...
    promptOverlay: [
      ...
      'Foreground social/content commits requested by the user and saved/authorized login session reuse can run with visible progress; high-consequence commits such as payment, purchase, transfer, first-time login, QR/OTP/captcha/passkey/security verification, account switching, credential storage, third-party authorization, legal filing/signature, ambiguous submit, and destructive/system changes still require confirmation or handoff.',
