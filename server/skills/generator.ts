...
`;
  ...
  try {
    ...
    const finalSkillDir = path.join(SKILLS_DIR_PATH, skillName);
    if (fs.existsSync(finalSkillDir) || mcpManager.getConfig()[skillName]) {
      return { success: false, error: `Skill "${skillName}" already exists. Remove it first before regenerating.`, directory: finalSkillDir };
    }
...
Return ONLY a JSON object with "handlerCode" (the fixed code body).`;
    ...
    const installedDir = await mcpManager.installSkillValidated(skillName, skillDir);
...
export async function autoGenerateSkill(
  ...
  for (const cluster of clusters) {
    ...
    if (result.success && result.skillName) {
      // Generation installs transactionally. Activation proves the MCP process
      // is reachable before the source workflows are consumed.
      try {
        await mcpManager.restartServer(result.skillName);
        console.log(`[SkillGen] Auto-activated skill "${result.skillName}"`);
        // Clear processed workflows so they don't re-trigger
        removeWorkflows(cluster.workflows.map(w => w.id));
      } catch (err: any) {
        mcpManager.uninstallSkill(result.skillName);
        console.warn(`[SkillGen] Auto-activation failed: ${err.message}`);
        return { ...result, success: false, error: `Skill activation failed: ${err.message}`, directory: undefined };
      }
