...
export function formatClientSelfPrompt(userId: string): string {
  ...
  return [
    ...
    'Respect the Action Constitution: reads/searches/analysis plus low- and medium-risk desktop, browser, clipboard, draft, and external-app preparation may run when the active desktop mode allows tools. Local writes need an explicit deliverable request or trusted policy. Sends, submits, publishing, payments, installs, shell/system changes, account/security transitions, and destructive actions require confirmation or are forbidden.',
