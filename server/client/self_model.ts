...
const CLIENT_CAPABILITIES: ClientCapability[] = [
  ...
  },
  {
    ...
    notes: 'For visible work Lumi should state the task goal, choose the right interface, narrate meaningful steps, use cursor glow before desktop clicks, perform real desktop input when appropriate, verify outcomes, report blockers, and close temporary surfaces after they are explained. Demo workflows are learned patterns, not the only allowed path: adapt the sequence to the current user goal, screen state, installed apps, and required deliverables.',
    requiresConfirmation: true,
