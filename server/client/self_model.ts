...
const CLIENT_CAPABILITIES: ClientCapability[] = [
  ...
  },
  {
    ...
    kind: 'system',
    actions: ['client_get_state', 'client_action', 'desktop_cursor_glow_show', 'desktop_cursor_glow_update', 'desktop_cursor_glow_click', 'desktop_mouse_click_at', 'desktop_active_window', 'desktop_capture_screen'],
    notes: 'For visible work Lumi should state the task goal, choose the right interface, inspect the screen/current window, move the visible cursor to the real target before desktop clicks, perform real desktop input when appropriate, verify outcomes, report only results/blockers/needed confirmations, and close temporary surfaces after they are explained. Demo workflows are learned patterns, not the only allowed path: adapt the sequence to the current user goal, screen state, installed apps, and required deliverables.',
    requiresConfirmation: true,
  ...
  },
  {
    ...
    notes: 'Persistent task hub for current-stage work takeover. Lumi can turn WeChat messages or user instructions into tracked takeover tasks with status, source, category, industry parameters, next actions, drafts, artifacts, blockers, risks, and confirmation boundaries, then continue them later. Use work_takeover_task_orchestrate to bridge a persisted task into a reusable execution plan: select capabilities by goal/artifacts/context, choose next tools/workflows, and record the plan without turning an industry demo into a fixed script. Use work_takeover_task_execute_step to safely prepare one plan step, write artifacts/drafts/verification notes back to the task, and stop before any external side effect that requires confirmation. Use work_takeover_task_advance when the user says continue/next step: it selects the next unprepared execution step from history, prepares it, records progress, and exports a packet when the safe steps are already covered. Use work_takeover_task_autorun for a bounded real-loop smoke test from message/clipboard/task: create or continue a task, advance a few safe steps, prepare real local ecommerce/short-video packages for store/account/video_publish tasks and renovation design packages for design_delivery tasks, stop at blockers or confirmation boundaries, export a local packet, and report what happened. Use work_takeover_task_prepare_ecommerce_growth when a persisted ecommerce, account, or short-video task needs the real local package: store audit, content matrix, scripts, image/video prompts, publish draft, customer-service/WeChat draft, operation report, and verification record. Use work_takeover_task_prepare_design_delivery when a persisted renovation/design delivery task needs the real local package: proposal, budget, PPT/PDF, CAD DXF, Revit/Dynamo handoff, WeChat draft, and verification record. Use work_takeover_task_verify_result after visible desktop or external tool work to check active window/processes, screenshot evidence, local paths, artifact content terms, drafts, confirmation boundaries, and task-center state before claiming success. Use work_takeover_task_export_packet to materialize the task into a local desktop packet with summary, plan, artifacts, drafts, risks, verification checklist, and JSON. Use work_takeover_task_run_suggested_tool only when a specific plan-suggested tool and arguments are ready; the underlying tool keeps its own confirmation behavior.',
  ...
  },
  {
    ...
    kind: 'external_app',
    actions: ['floorplan_extract_geometry', 'ocr_image_file', 'cad_generate_dxf', 'design_delivery_workflow', 'desktop_open', 'desktop_run_command'],
    notes: 'Lumi can extract CAD-ready geometry from plan images, generate structured DXF draft files with doors/windows/dimensions, include CAD drafts in renovation design delivery packages, and hand the DXF to detected desktop CAD software such as FreeCAD/AutoCAD-compatible tools when available. Lumi should not substitute a browser preview for CAD handoff when a real CAD application is available. Production drawings still require user review and confirmed site dimensions.',
    requiresConfirmation: true,
    stateKeys: ['permissions', 'tools'],
  },
  {
...
export function formatClientSelfPrompt(userId: string): string {
  ...
  return [
    ...
    'When the user asks Lumi to handle, reply to, classify, or take over a WeChat message, use work_takeover_task_autorun when they want a bounded safe loop from message/clipboard to local packet. For design-delivery messages, autorun should prepare the real local renovation package and record its verification unless the user asks not to. Use work_takeover_task_from_wechat/from_clipboard for manual creation, then continue/orchestrate/advance when they want more control. Run a specific suggested tool only when arguments and confirmation boundaries are clear.',
