...
const CLIENT_CAPABILITIES: ClientCapability[] = [
  ...
  },
  {
    ...
    notes: 'Lumi distinguishes visible window state, hidden-to-background resident client state, backend process health, launch-at-login, close-to-background, and autonomous workflow execution. Resident background availability requires the desktop client/server to be alive; autonomous background work additionally requires desktop mode/autonomy policy, idle/time/token gates, and a user-confirmed workflow. Verify runtime state before promising that Lumi will keep working after the window is hidden or after restart.',
    requiresConfirmation: true,
  ...
  },
  {
    ...
    notes: 'When work involves WeChat, store backends, creator platforms, or other account surfaces, Lumi can restore and use already logged-in taskbar/background windows or saved browser profiles for visible safe preparation work under the active desktop mode. If the exact local app path is unknown, use desktop_list_apps before desktop_open instead of guessing install paths or generating a one-off skill. With explicit authorization, Lumi can learn a generic website login, store encrypted credentials locally, or reuse a browser/session-only login profile after the user completes QR/OTP/captcha/passkey checks. In foreground user-present execution, ordinary messages, comments, replies, and non-commercial content posts can proceed when the user asked for them. It must stop for user confirmation or handoff at first-time login, QR/OTP/biometric checks, account switching, third-party authorization, saving credentials, payment, purchase, transfer, legal filing/signature, or other high-consequence commits. The learned behavior is to continue from existing sessions instead of pretending that a local HTML page or a fresh browser tab is real account control.',
    requiresConfirmation: true,
  ...
  },
  {
    ...
    kind: 'external_app',
    actions: ['wechat_intake_analyze', 'wechat_intake_from_clipboard', 'work_takeover_task_from_wechat', 'work_takeover_task_from_clipboard', 'wechat_prepare_reply', 'wechat_copy_reply_draft', 'wechat_send_message', 'desktop_active_window', 'desktop_list_apps', 'desktop_open', 'desktop_mouse_click_at', 'desktop_cursor_glow_show', 'desktop_cursor_glow_update', 'desktop_cursor_glow_click', 'desktop_cursor_glow_hide', 'desktop_run_command'],
    notes: 'Lumi can triage user-provided or copied WeChat messages into current-stage work takeover tasks, extract key amounts/deadlines/people, persist the task, prepare next actions and reply drafts, copy drafts, and send ordinary foreground user-requested WeChat messages through the dedicated send tool when the user is present. For visible desktop work and handoff, Lumi should first restore an already running personal WeChat/Weixin window from the taskbar/background with visible focus, then use desktop_list_apps/desktop_open to launch personal WeChat from the native app index, and only then fall back to enterprise WeChat/WeCom. If WeChat is logged in, Lumi may continue safe draft/preparation work inside that session and use the virtual cursor path for explicit ordinary sends; QR login, verification, account switching, payments, legal/contractual commitments, and other high-consequence sends remain confirmation/handoff boundaries. It should not claim to send messages unless the foreground send tool or another confirmed integration actually completed.',
    requiresConfirmation: true,
    stateKeys: ['permissions', 'tools'],
  },
  {
    ...
    notes: 'Lumi can extract CAD-ready geometry from plan images, generate structured DXF draft files with doors/windows/dimensions, generate AutoCAD LISP/SCRIPT playback so entities appear stroke by stroke, execute the script through AutoCAD /b, and verify completion with a marker file plus desktop process/window evidence. Use desktop_list_apps to find installed CAD launchers before guessing AutoCAD paths. Lumi should not substitute a browser preview for CAD handoff when a real CAD application is available. Production drawings still require user review and confirmed site dimensions.',
    requiresConfirmation: true,
    stateKeys: ['permissions', 'tools'],
  },
  {
  ...
  return {
    ...
    autonomyBoundary: {
      ...
      confirmFirst: [
        ...
        'Operate shell/system commands, first-time login/account-security transitions, CAD app execution, ambiguous external submits, high-consequence external commits, or file writes without explicit deliverable intent.',
      ...
      forbidden: [
        ...
        'Purchase/pay/transfer, place or cancel real brokerage orders, change orders/prices/inventory/ad spend, perform account-security transitions, file/sign legal commitments, or run ambiguous external submits without confirmation.',
...
export function formatClientSelfPrompt(userId: string): string {
  ...
  return [
    ...
    'Respect the Action Constitution: reads/searches/analysis plus low- and medium-risk desktop, browser, clipboard, draft, external-app preparation, user-requested foreground social/content commits, and stock watch actions such as quotes, K-lines, sectors, news, watchlists, alerts, risk plans, and paper trading may run when the active desktop mode allows tools. Local writes need an explicit deliverable request or trusted policy. Payments, purchases, transfers, real brokerage buy/sell/cancel-order actions, order/price/inventory/ad-spend changes, ambiguous external submits, installs, shell/system changes, account/security transitions, legal filings/signatures, and destructive actions require confirmation or are forbidden.',
