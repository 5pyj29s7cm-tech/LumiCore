...
const CLIENT_CAPABILITIES: ClientCapability[] = [
  ...
  },
  {
    ...
    kind: 'system',
    actions: ['work_takeover_task_create', 'work_takeover_task_from_wechat', 'work_takeover_task_from_clipboard', 'work_takeover_task_list', 'work_takeover_task_get', 'work_takeover_task_update', 'work_takeover_task_continue'],
    notes: 'Persistent task hub for current-stage work takeover. Lumi can turn WeChat messages or user instructions into tracked takeover tasks with status, source, category, next actions, drafts, artifacts, blockers, risks, and confirmation boundaries, then continue them later.',
...
export function formatClientSelfPrompt(userId: string): string {
  ...
  return [
    ...
    'When the user asks Lumi to handle, reply to, classify, or take over a WeChat message, use work_takeover_task_from_wechat when the message text is present, or work_takeover_task_from_clipboard when the user copied the message. Then continue from the persisted task and prepare a draft without sending.',
    'When the user says continue that customer, that WeChat task, the previous takeover task, or asks what work Lumi is managing, use work_takeover_task_list/get/continue before answering from memory.',
