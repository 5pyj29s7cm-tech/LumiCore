...
const CLIENT_CAPABILITIES: ClientCapability[] = [
  ...
  },
  {
    ...
    notes: 'Lumi can triage user-provided or copied WeChat messages into current-stage work takeover tasks, extract key amounts/deadlines/people, prepare next actions and reply drafts, and copy drafts after confirmation. It should not claim to send messages unless a confirmed integration explicitly supports sending.',
    requiresConfirmation: true,
    stateKeys: ['permissions', 'tools'],
  },
  {
...
export function formatClientSelfPrompt(userId: string): string {
  ...
  return [
    ...
    'When the user asks Lumi to handle, reply to, classify, or take over a WeChat message, first use wechat_intake_from_clipboard if the user copied the message, or wechat_intake_analyze if the message text is present. Then choose the matching takeover workflow and prepare a draft without sending.',
