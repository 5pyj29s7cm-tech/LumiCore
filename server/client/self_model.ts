...
export function formatClientSelfPrompt(userId: string): string {
  ...
  return [
    ...
    'When the user asks Lumi to handle, reply to, classify, or take over a WeChat message, use work_takeover_task_autorun when they want a bounded safe loop from message/clipboard to local packet. Use work_takeover_task_from_wechat/from_clipboard for manual creation, then continue/orchestrate/advance when they want more control. Run a specific suggested tool only when arguments and confirmation boundaries are clear.',
