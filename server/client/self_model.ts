...
const CLIENT_CAPABILITIES: ClientCapability[] = [
  ...
  },
  {
    ...
    kind: 'system',
    actions: ['design_delivery_workflow', 'design_delivery_panel', 'close_design_delivery_panel', 'desktop_show_lumi_window', 'desktop_set_wallpaper_mode', 'desktop_open', 'desktop_clipboard_write', 'create_ppt', 'create_pdf', 'cad_generate_dxf'],
    notes: 'In this stage, when the user asks Lumi to take over a renovation/design delivery task, Lumi can generate a local proposal package, budget/material list, PPTX presentation, PDF report, CAD DXF and SVG preview, Revit/Dynamo handoff files, and a WeChat delivery draft. Production drawings still require confirmed site dimensions, structure, utilities, and user sign-off.',
    requiresConfirmation: true,
    stateKeys: ['surfaces', 'windows', 'voice', 'tools', 'permissions'],
  },
  {
  ...
  },
  {
    ...
    notes: 'Lumi can triage user-provided or copied WeChat messages into current-stage work takeover tasks, extract key amounts/deadlines/people, persist the task, prepare next actions and reply drafts, and copy drafts after confirmation. It should not claim to send messages unless a confirmed integration explicitly supports sending.',
    requiresConfirmation: true,
    stateKeys: ['permissions', 'tools'],
  },
  {
    ...
    kind: 'external_app',
    actions: ['floorplan_extract_geometry', 'ocr_image_file', 'cad_generate_dxf', 'design_delivery_workflow'],
    notes: 'Lumi can extract CAD-ready geometry from plan images, generate structured DXF draft files with doors/windows/dimensions, and include CAD drafts in renovation design delivery packages after confirmation. Production drawings still require user review and confirmed site dimensions.',
    requiresConfirmation: true,
    stateKeys: ['permissions', 'tools'],
  },
  {
