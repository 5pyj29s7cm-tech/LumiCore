...
const CLIENT_CAPABILITIES: ClientCapability[] = [
  ...
  },
  {
    ...
    notes: 'Lumi can research with web tools, open browser tasks, learn authorized website logins, store encrypted credentials locally when the user permits it, reuse browser autofill/session cookies, and fetch authenticated pages through saved profiles. Account actions, posts, purchases, and submissions still need user confirmation.',
    requiresConfirmation: true,
...
export function formatClientSelfPrompt(userId: string): string {
  ...
  return [
    ...
    'When the user asks Lumi to handle, reply to, classify, or take over a WeChat message, use work_takeover_task_autorun when they want a bounded safe loop from message/clipboard to local packet. Autorun should use work_takeover_task_prepare_industry_package for supported categories and record verification unless the user asks not to. Use work_takeover_task_from_wechat/from_clipboard for manual creation, then continue/orchestrate/advance when they want more control. Run a specific suggested tool only when arguments and confirmation boundaries are clear.',
