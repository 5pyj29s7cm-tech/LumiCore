...
impl ShutdownBarrier {
    ...
    pub fn prevents_restart(&self) -> bool {
        self.state.lock().map(|state| state.running || state.saved).unwrap_or(true)
    }
    ...
    pub fn run(&self, save: impl FnOnce() -> Result<(), String>) -> Result<(), String> {
        ...
        if state.running {
            while state.running {
                state = self.completed.wait(state).map_err(|error| error.to_string())?;
            }
            return state.result.clone().unwrap_or_else(|| Err("Shutdown did not produce a save receipt".into()));
        }
        state.running = true;
        drop(state);
        let result = save();
...
fn parse_save_receipt(raw: &[u8], expected_pid: u32) -> Result<(), String> {
    let header_end = raw.windows(4).position(|bytes| bytes == b"\r\n\r\n")
...
pub fn request_backend_save(expected_pid: u32) -> Result<(), String> {
    ...
    let token = identity.get("token").and_then(Value::as_str)
        .ok_or_else(|| "Native shutdown authentication is unavailable".to_string())?;
    let proof = identity.get("desktopSessionProof").and_then(Value::as_str)
        .ok_or_else(|| "Native shutdown session proof is unavailable".to_string())?;
    if [token, proof].iter().any(|value| value.is_empty() || value.len() > 16 * 1024 || value.contains(['\r', '\n'])) {
        return Err("Native shutdown credentials are invalid".into());
    }
    let port = std::env::var("PORT").ok().filter(|value| !value.trim().is_empty())
        .map(|value| value.trim().parse::<u16>())
        .transpose().map_err(|_| "Backend port is invalid".to_string())?.unwrap_or(3000);
    if port == 0 { return Err("Backend port is invalid".into()); }
    ...
    stream.write_all(request.as_bytes()).and_then(|_| stream.write_all(&body))
        .and_then(|_| stream.flush()).map_err(|error| format!("Cannot send backend save request: {error}"))?;
    let mut response = Vec::new();
    stream.take(65_537).read_to_end(&mut response)
        .map_err(|error| format!("Still waiting for backend save confirmation; closing was cancelled: {error}"))?;
    if response.len() > 65_536 { return Err("Backend save receipt exceeded its limit".into()); }
...
mod tests {
    ...
    fn requires_saved_receipt_from_the_owned_backend() {
        assert!(parse_save_receipt(b"HTTP/1.1 200 OK\r\n\r\n{\"ok\":true,\"status\":\"saved\",\"pid\":42}", 42).is_ok());
        assert!(parse_save_receipt(b"HTTP/1.1 200 OK\r\n\r\n{\"ok\":true,\"status\":\"saved\",\"pid\":41}", 42).is_err());
    ...
    fn duplicate_quit_waits_for_the_same_save() {
        ...
        let duplicate = std::thread::spawn(move || done_tx.send(second.run(|| panic!("must share first save"))).unwrap());
